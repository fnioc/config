...
A standalone, runnable `@fnconfig/config` consumer that uses the
`@fnconfig/transformer` ts-patch plugin for **type-driven** configuration —
...
`@fnconfig/config`'s throwing stub — the honest, loud failure that proves the
