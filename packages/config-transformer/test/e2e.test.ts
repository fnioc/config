...
// a temp project that imports `@fnconfig/transformer` as a ts-patch plugin, then
...
beforeAll(() => {
  projDir = mkdtempSync(join(tmpdir(), "fnconfig-tsp-e2e-"));
  const nm = join(projDir, "node_modules");
  mkdirSync(join(nm, "@fnconfig"), { recursive: true });
  ...
  link(PKG_ROOT, join(nm, "@fnconfig", "transformer"));
  link(join(REPO_ROOT, "packages", "config"), join(nm, "@fnconfig", "config"));
  link(join(REPO_ROOT, "packages", "core"), join(nm, "@fnconfig", "core"));
...
`,
  ...
  writeFileSync(
    ...
    JSON.stringify({
      compilerOptions: {
        ...
        skipLibCheck: true,
        noEmitOnError: false,
        plugins: [{ transform: "@fnconfig/transformer", import: "transform" }],
      },
      include: ["src/**/*"],
    }),
  );
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "fnconfig-e2e-fixture", private: true, type: "module" }),
  );
});
...
describe("ts-patch production e2e (ESM)", () => {
  test("tspc compiles and emits the lowered .withSchema contract", () => {
    ...
    expect(emitted).toContain(`import { OPTIONAL } from "@fnconfig/config"`);
