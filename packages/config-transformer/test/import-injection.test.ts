...
describe("OPTIONAL import injection", () => {
  test("injects the named import when absent", () => {
    ...
    expect(output).toContain(`import { OPTIONAL } from "@fnconfig/config"`);
  ...
  test("does not duplicate an existing named import", () => {
    ...
    expect(diagnostics).toHaveLength(0);
    const occurrences = output.split(`import { OPTIONAL } from "@fnconfig/config"`).length - 1;
  ...
  test("honors an aliased named import", () => {
    const { output, diagnostics } = transform(
      fixture(`
        import { OPTIONAL as OPT } from "@fnconfig/config";
        interface T { ssl?: boolean }
        const b = new ConfigurationBuilder().withType<T>();
      `),
    );
  ...
  test("honors a namespace import", () => {
    const { output, diagnostics } = transform(
      fixture(`
        import * as cfg from "@fnconfig/config";
        interface T { ssl?: boolean }
        const b = new ConfigurationBuilder().withType<T>();
      `),
    );
