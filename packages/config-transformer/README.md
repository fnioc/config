# @fnconfig/transformer

The `@fnconfig` [ts-patch](https://github.com/nonara/ts-patch) transformer. It
...
The runtime (`@fnconfig/config`) owns all coercion; this package only
synthesizes the `Schema` literal `withType<T>()` stands in for. `.withType`
itself is `@fnconfig/config`'s opt-in Tier 2 authoring surface (`import
"@fnconfig/config/with-type-augment"`); without this transformer configured,
calling it hits a loud throwing stub — never a silent, un-coerced builder.

## Setup

Install alongside `@fnconfig/config` and wire the plugin into your
...
stays `Host`). An injected `import { OPTIONAL } from "@fnconfig/config"` is added
