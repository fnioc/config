...
// OPTIONAL is the `unique symbol` re-exported from the @fnconfig/config BARREL
// (`export { OPTIONAL } from "@fnconfig/config"`). Whenever a wrapper is
// emitted, the file needs a binding for that symbol. We resolve the binding
// once per file up front:
//
//   1. an existing NAMED import of `OPTIONAL` from "@fnconfig/config" (alias
//      honored -- use the local name), or
//   2. an existing NAMESPACE import from "@fnconfig/config" (`<ns>.OPTIONAL`), or
//   3. none -- fall back to a bare `OPTIONAL` identifier and flag that an
//      `import { OPTIONAL } from "@fnconfig/config";` must be prepended IF a
//      wrapper actually lowers.
//
// "From @fnconfig/config" means the barrel specifier EXACTLY; the subpath
// ".../with-type-augment" does not export OPTIONAL.

import ts from "typescript";

const CONFIG_BARREL = "@fnconfig/config";
...
export interface OptionalRef {
  ...
  /** Whether a `import { OPTIONAL } from "@fnconfig/config"` must be prepended. */
...
export function resolveOptionalBinding(
  ...
  for (const statement of sourceFile.statements) {
    ...
    // Namespace import: `import * as cfg from "@fnconfig/config"`.
...
/**
 * Prepend `import { OPTIONAL } from "@fnconfig/config";` to `sourceFile` iff at
