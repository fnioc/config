// Tier 2 seam -- `.withType<T>()`, contributed later by @fnconfig/transformer.
//
...
// "@fnconfig/config/with-type-augment" to bring the declaration (and the
// throwing stub) into scope. The real @fnconfig/transformer will replace this
...
// "@fnconfig/config/configuration-builder" (the same one the provider packages
...
declare module "@fnconfig/config/configuration-builder" {
  interface ConfigurationBuilder<T = IndexedSection> {
    /**
     * TIER 2 -- contributed by @fnconfig/transformer, which rewrites this call
...
ConfigurationBuilder.prototype.withType = function (): never {
  throw new Error(
    "withType<T>() requires @fnconfig/transformer's compile-time transform to run. "
