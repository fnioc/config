...
// covers the core barrel in source mode; this file covers what that one
// structurally cannot:
//
...
//      bundling with @fnconfig/config kept external, and the `declare module`
...
describe("cross-package public surface (built dist)", () => {
  ...
  test("the with-type-augment subpath ships its throwing stub in dist and throws under node", async () => {
    await import("@fnconfig/config/with-type-augment");
    const builder = new ConfigurationBuilder();
    assert.equal(typeof builder.withType, "function");
    assert.throws(() => builder.withType(), /@fnconfig\/transformer/);
  });
