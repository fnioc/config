// Build @fnioc/config-json for publication.
//
...
// 1. JS bundle — @fnioc/config external (a peer dep — consumers have it).
...
if (!js.success) {
  ...
  throw new Error("@fnioc/config-json: bun build failed");
}
...
if (dts.status !== 0) {
  throw new Error("@fnioc/config-json: rollup d.ts bundling failed");
}
